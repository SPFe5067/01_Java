...
public class StudantService {
	 ...
	 * - 검색할 이름을 입력받아 studantList에서 꺼내온
	 * Studant 객체의 name 값이 같은지 비교
	 * 
	 * 일치하는 경우 Studant객체 출력
	 * 일치하는게 없다면 검색결과 없습니다 출력
	 * 
	 */
	 ...
	 * 
	 * 문자열 입력받아 studantList에서 꺼내온 Studant객체의 name값에 포함되는 문자열인지 검사
	 * 
	 * -포한되는 학생 객체를 찾은 경우 Studant 객체 출력
	 * -없다면 "검색 결과가 없습니다" 출력
	 * 
	 * 
	 */
	 ...
	 * Studant에 Comparable 인터페이스를 상속받아 오버라이딩한 compareTo()에
	 * 정의한 대로 정렬 됨 (나이 오름차순, 내림차순..)
	 * 
	 ...
	 * 즉시 사용 가능(한번만 사용할 Comparator 등을 정의할 때 유용함)
	...
	public void sortByName() {
		Collections.sort(studantList, new Comparator<Studant>() {
