//예제 코드 작성용 클래스(기능용/메서드용)
package edu.kh.op.ex;

import java.util.Scanner;

public class OpExemple {

	//ex1() method : OpExample 이라는 클래스에 포함된 메서드
	public void ex1() {
		
		//증감 연산자 (++, --)
		//1을 더하거나 감소시키는 연산자
		
		int i1 = 10;
		int i2 = 10;
		
		i1++;
		i2--;
		System.out.println("i1 : " + i1);
		System.out.println("i2 : " + i2);
		
		//전위 연산
		int t1 = 5;
		
		System.out.println(++t1 + 5);
		
		
		//후위 연산
		int t2 = 3;
		System.out.println(t2-- + 2);
		// 				  3 + 2 = 5
		// 				  3-- 	= 2
		System.out.println(t2);
		
		//test
		int a = 3;
		int b = 5;
		int c = a++ + --b;
		// 최종적으로 a,b,c는 각각 얼마라고 출력될까?
		System.out.printf("%d / %d / %d\n", a, b, c);
		//                  4    4   7
				
		
	}
	
	public void ex2() {
		
		//비교 연산자 : > , < , >= , <= , == , !=
		//-비교 연산자의 결과는 항상 논리값(true/false)
		
		int a = 10;
		int b = 20;
		
		System.out.println( (a == b) == false); // = true
		
		int c = 4;
		int d = 5;
		System.out.println( (++c != d) == (--c != d));
		//   				  5     5       4     5
		//						false  ==   true
		//							  false
		System.out.println();
		
		System.out.println(100 > 99);
		System.out.println(100 >= 100);
		System.out.println(99 <= 88);
		
		
		
	}
	
	public void ex3() {
		//논리 연산자 : && , ||
	System.out.println();	
		// &&(and) : 둘다 true 면 true, 나머지는 false
		int a= 101;
		//a는 100 이상 이면서 짝수인가?
		System.out.println( (a>=100) && (a % 2 == 0)); // false
		
		// ||(or)  : 둘중 하나라도 true 면 true, 나머지는 false
		System.out.println( (a>=100) || (a % 2 == 0)); // true
		
		//b는 1부터 10까지 숫자 범위에 포함되어 있는가?
		//1~10 -> b는 1보다 크거나 같으면서, 10보다 작거나 같다.
		int b = 5;
		System.out.println( (b >= 1) && (b <= 10) );
		System.out.println( (b >= 1) || (b <= 10) );
		
		//c는 10을 초과하거나 짝수인가?
		int c = 10;
		System.out.println((c >= 10) || (c % 2 == 0));
	
	
	
	
	}
	
	public void ex4() {
		//삼항 연산자
		//삼항 연산자 : 조건식 ? (true일때 수행될 값) : (false일때 수행될 값)
    	//	*조건식 : 연산결과가 true/false 인 공식.
		int num = 30;
		
		//num 이 30보다 크다면 "num은 30보다 큰 수 이다."
		// 아니라면 "num은 30보다 작은 수 이다." 출력.
		
		String result = num > 30 ? "num은 30보다 큰 수이다." : "num은 30보다 작은 수이다.";
		
		System.out.println(result);
		
		
		//test
		
		//입력받은 정수가 음수인지 양수인지 구분
		//단, 0은 양수로 처리
		
		Scanner sc = new Scanner(System.in);
		
		System.out.println("수를 입력 : ");
		
		int i1 = sc.nextInt();
		
		String result2 = i1 >= 0 ? "양수 입니다." : "음수 입니다.";

		
		
	}
}
