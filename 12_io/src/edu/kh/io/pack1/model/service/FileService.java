...
public class FileService {
	...
	public void method1() {
		...
		if(!directory.exists()) {//만약에 폴더가 좋재하지 않으면
		//폴더를 모두 생성
			directory.mkdirs();
	...
	public void method2() {
		//file 생성
		//문자열에서 "\"를 표기하는 방법은 두번 사용하는 것. == "\\"
		//"\"는 이스케이프 문자로 사용됨
		File file = new File("\\io_test\\20250305\\파일생성.txt");
		
		if(!file.exists()) {
			try {
				//File 클래스의 메서드는 대부분 IOException을 발생 시킴
				//IOException는 반드시 예외처리를 해야하는 Checked Exception이다
				//예외처리 필수.
				...
				e.printStackTrace();//예외에 대한 모든 정보 + 예외가 발생한 위치까지의 메서드 추적
			}
		}
	...
	public void method3() {
		...
		//System.out.println(date);
			...
			//구하려면 폴더 안의 모든 파일을 순회하며 각 파일 크기를 합산해서 써야함.
